(function () {
  ...
  function registerSubscription(payload) {
    ...
    var notExpired = !expiresAt || new Date(expiresAt).getTime() > Date.now();
    var premiumActive = Boolean(payload.premium_active || payload.premiumActive || planType === "premium") && notExpired;
  ...
  function registerEligibilityEvent(payload) {
    ...
    var nextDate = dateOnly(payload.eligibility_date || payload.eligibilityDate);
    var dateChanged = existing && existing.eligibility_date && nextDate && existing.eligibility_date !== nextDate;
    var row = Object.assign(existing || {
      ...
      eligibility_date: nextDate,
      eligibility_reason: clean(payload.eligibility_reason || payload.eligibilityReason),
      waiting_period: clean(payload.waiting_period || payload.waitingPeriod || payload.requiredWaitingPeriodLabel),
      ...
      court_id: clean(payload.court_id || payload.courtId),
      rule_set_id: clean(payload.rule_set_id || payload.ruleSetId),
  ...
  function runPacketAbandonmentCheck(now) {
    ...
    var typeMap = { 3: "packet_incomplete_3_day", 7: "packet_incomplete_7_day", 14: "packet_incomplete_14_day" };
    if (!typeMap[days]) return [];
    var markerKey = "recordwatchPacketReminder" + days + "Sent";
    if (localStorage.getItem(markerKey) === "true") return [];
    var subscription = loadSubscriptions()[0] || { user_id: currentUserId(), case_id: "demo-case", notification_email: localStorage.getItem("email"), notification_phone: localStorage.getItem("phone"), notify_email: true, notify_sms: false, premium_active: false };
    localStorage.setItem(markerKey, "true");
    return createNotification(subscription, typeMap[days], { days: days, notification_date: markerKey });
  }
