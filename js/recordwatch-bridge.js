(function () {
  ...
  function getCurrentEligibilityResult(caseData) {
    ...
    if (active && window.RecordPathEligibilityEngine && typeof window.RecordPathEligibilityEngine.resolveEligibilityForCase === "function") {
      ...
      return {
        ...
        completionDateUsed: resolved.dateUsedForCalculation || "",
        completionDateField: resolved.dateUsedForCalculationField || "",
        waitingPeriodText: resolved.requiredWaitingPeriodLabel || "",
        requiredWaitingPeriodLabel: resolved.requiredWaitingPeriodLabel || "",
    ...
    return {
      ...
      completionDate: clean(stored.date_used_for_calculation || stored.completion_date_used || stored.completion_date),
      completionDateUsed: clean(stored.completion_date_used || stored.date_used_for_calculation || stored.completion_date),
      ...
      requiredWaitingPeriodLabel: clean(stored.required_waiting_period || stored.waiting_period),
  ...
  function persistResolvedEligibility(caseData, result, userId) {
    ...
    var storage = {
      ...
      required_waiting_period: result.requiredWaitingPeriodLabel || result.waitingPeriodText || raw.requiredWaitingPeriodLabel || "",
      waiting_period: result.requiredWaitingPeriodLabel || result.waitingPeriodText || raw.requiredWaitingPeriodLabel || "",
  ...
  function registerRecordWatchEligibility(caseData) {
    ...
    window.RecordWatchNotifications.registerEligibilityEvent({
      ...
      waitingPeriod: result.requiredWaitingPeriodLabel || result.waitingPeriodText || "",
      requiredWaitingPeriodLabel: result.requiredWaitingPeriodLabel || result.waitingPeriodText || "",
      completionDateUsed: result.completionDateUsed || result.completionDate || result.dateUsedForCalculation,
      completionDate: result.completionDate || result.completionDateUsed || result.dateUsedForCalculation,
